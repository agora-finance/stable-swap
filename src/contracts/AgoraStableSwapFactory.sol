...
/// @author Agora
contract AgoraStableSwapFactory is AgoraAccessControl, Initializable {
    ...
    function initialize(InitializeParams memory _params) external initializer {
        ...
        // We temporarily set the upgrader role to the sender to set the pair implementation address
    ...
    /// @dev Only the Admin can set the default roles
    ...
    /// @dev Only the whitelister can set the approved deployers
    ...
    /// @dev Only the upgrader can update the pair implementation address
    ...
    function setStableSwapImplementation(address _newImplementation) public {
        // Checks: Only the upgrader can set the implementation
    ...
    function createPair(CreatePairArgs memory _pairArgs) external returns (address pair) {
        // Checks: Only the manager can set the default values
    ...
    /// @notice Emits when a new pair is created
    /// @param token0 The address of the first token in the pair
    /// @param token1 The address of the second token in the pair
